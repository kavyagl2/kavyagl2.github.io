...
Notations from Tang et al.:
- \( m \): word embedding size
- \( n \): number of hidden units
- \( K \): vocabulary size
- \( h^l_i \): hidden state at step \( i \) of layer \( l \)
- \( E \in \mathbb{R}^{m \times K} \): word embedding matrix
- \( W \in \mathbb{R}^{n \times m} \), \( U \in \mathbb{R}^{n \times n} \): weight matrices
- \( E x_i \): embedding of \( x_i \)
- \( e_{pos,i} \): positional embedding at position \( i \)
...
- First layer: \( h^0_i = W E x_i + e_{pos,i} \)
- Decoder: \( h^l_i = h^{l-1}_i + f(\text{self-attention}(h^{l-1}_i)) \)
